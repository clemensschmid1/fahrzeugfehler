              ...
              return logoUrl ? (
                ...
                >
                  <div className="relative group">
                    ...
                    <div className="relative bg-gradient-to-br from-white/20 via-white/15 to-white/10 dark:from-white/15 dark:via-white/10 dark:to-white/5 backdrop-blur-xl rounded-3xl p-6 sm:p-8 md:p-10 border-2 border-white/40 dark:border-white/30 shadow-2xl group-hover:border-white/50 dark:group-hover:border-white/40 transition-all duration-500 group-hover:shadow-[0_0_50px_rgba(255,255,255,0.4)] group-hover:scale-105">
                      {/* Inner glow ring */}
                      <div className="absolute inset-2 rounded-2xl border border-white/20 dark:border-white/10"></div>
                      {/* Subtle shine effect */}
                      <div className="absolute inset-0 bg-gradient-to-br from-white/30 via-transparent to-transparent rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                      
                      <img
                        src={logoUrl}
                        alt={`${brand.name} logo`}
                        className="relative h-28 sm:h-36 md:h-44 lg:h-52 object-contain filter drop-shadow-[0_0_40px_rgba(0,0,0,0.6)] group-hover:scale-110 transition-transform duration-500"
                        loading="eager"
                        decoding="async"
                        fetchPriority="high"
                        onError={(e) => {
        ...
        <div className="mb-12">
          <div className="relative max-w-2xl mx-auto">
            <input
              ...
              className="w-full px-6 py-4 text-lg border-2 border-slate-300 dark:border-slate-700 rounded-xl bg-white/95 dark:bg-slate-900/95 backdrop-blur-sm text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-4 focus:ring-blue-500/20 dark:focus:ring-blue-500/20 focus:border-blue-500 dark:focus:border-blue-500 transition-all shadow-lg hover:shadow-xl"
            />
            <svg
