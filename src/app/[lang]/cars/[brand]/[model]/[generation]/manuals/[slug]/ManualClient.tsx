...
export default function ManualClient({ brand, model, generation, manual, relatedManuals, lang, initialComments = [], user }: Props) {
  ...
  const headings = manual.content.match(/^#{1,3}\s+(.+)$/gm) || [];
  ...
  const handleShare = async (platform: string) => {
    ...
    const text = `${manual.title} - ${brand.name} ${model.name} ${generation.name}`;
        ...
        <div className="mt-12 p-6 bg-slate-50 dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button
              ...
              className="px-4 py-2 bg-blue-700 hover:bg-blue-800 text-white rounded-lg font-semibold transition-all flex items-center gap-2"
            >
              <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
            ...
            <button
              ...
              className="px-4 py-2 bg-slate-600 hover:bg-slate-700 text-white rounded-lg font-semibold transition-all flex items-center gap-2"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {!user ? (
            ...
            <form onSubmit={handleCommentSubmit} className="mb-8 bg-slate-50 dark:bg-slate-900/30 border border-slate-200 dark:border-white/10 rounded-2xl p-4 sm:p-6">
              <div className="flex items-start gap-3 sm:gap-4">
                <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-gradient-to-br from-blue-500 to-indigo-500 flex items-center justify-center text-white font-semibold text-xs sm:text-sm flex-shrink-0 shadow-sm">
                ...
                <div key={comment.id} className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-white/10 rounded-2xl p-4 sm:p-6 hover:shadow-md transition-all duration-200">
                  <div className="flex items-start gap-3 sm:gap-4">
                    <div className="flex-shrink-0">
                      <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-gradient-to-br from-blue-500 to-indigo-500 flex items-center justify-center text-white font-semibold text-xs sm:text-sm shadow-sm">
                        {initials}
                      </div>
                    </div>
                    <div className="flex-1 min-w-0">
