...
export default function ModelFaultsSection({ 
  ...
  useEffect(() => {
    const loadFaults = async () => {
      ...
      try {
        const url = selectedGenId
          ? `/api/cars/${brandSlug}/${modelSlug}/faults?generationId=${selectedGenId}&page=1&limit=${FAULTS_PER_PAGE}&lang=${lang}`
          : `/api/cars/${brandSlug}/${modelSlug}/faults?page=1&limit=${FAULTS_PER_PAGE}&lang=${lang}`;
    ...
    loadFaults();
  }, [selectedGenId, brandSlug, modelSlug, lang]);
  ...
  const loadMore = async () => {
    ...
    try {
      const url = selectedGenId
        ? `/api/cars/${brandSlug}/${modelSlug}/faults?generationId=${selectedGenId}&page=${page + 1}&limit=${FAULTS_PER_PAGE}&lang=${lang}`
        : `/api/cars/${brandSlug}/${modelSlug}/faults?page=${page + 1}&limit=${FAULTS_PER_PAGE}&lang=${lang}`;
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-4">
        <div>
          <h2 className="text-3xl font-black text-slate-900 dark:text-white mb-2">
            {t('Faults', 'Fehler')} - {modelName}
          </h2>
          <p className="text-slate-600 dark:text-slate-400">
            {selectedGeneration
              ? t(
                  `Showing ${faults.length} of ${totalCount.toLocaleString()} faults for ${selectedGeneration.generation_code || selectedGeneration.name}`,
                  `Zeige ${faults.length} von ${totalCount.toLocaleString()} Fehlern für ${selectedGeneration.generation_code || selectedGeneration.name}`
                )
              : t(
                  `Showing ${faults.length} of ${totalCount.toLocaleString()} faults for all generations`,
        ...
        <div className="mb-8">
          <div className="flex flex-wrap items-center gap-3">
            <span className="text-sm font-bold text-slate-700 dark:text-slate-300">
              {t('Filter by Generation:', 'Nach Generation filtern:')}
            </span>
            <button
              ...
              className={`px-4 py-2 rounded-lg font-bold text-sm border-2 transition-all duration-200 ${
                selectedGenId === null
                  ...
                  : 'bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 border-slate-300 dark:border-slate-600 hover:border-red-500 dark:hover:border-red-500'
              }`}
            >
