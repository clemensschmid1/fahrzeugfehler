...
export default function GenerationListClient({ brand, model, generations, totalFaultsCount = 0, initialFaults = [] }: Props) {
    ...
    <div className="min-h-screen bg-white dark:bg-black">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-20 md:py-28">
          ...
          >
            ...
            <p className="text-base sm:text-lg md:text-xl lg:text-2xl text-slate-300 dark:text-slate-400 mb-3 sm:mb-4 max-w-3xl mx-auto leading-relaxed px-2">
              Wählen Sie Ihre Generation
            </p>

            {model.description && (
              <p className="text-sm sm:text-base md:text-lg text-slate-400 max-w-2xl mx-auto mb-3 sm:mb-4 px-2">
                {model.description}
              </p>
            )}
            ...
            <div className="mt-6 sm:mt-8 flex flex-wrap items-center justify-center gap-3 sm:gap-4">
              <Link
                href={`/cars/${brand.slug}/${model.slug}/error-codes`}
                className="inline-flex items-center gap-2 px-5 sm:px-6 py-2.5 sm:py-3 min-h-[44px] bg-white/10 dark:bg-white/5 backdrop-blur-md border-2 border-white/30 dark:border-white/20 text-white font-bold rounded-lg sm:rounded-xl hover:bg-white/20 dark:hover:bg-white/10 hover:border-white/40 dark:hover:border-white/30 transition-all shadow-lg hover:shadow-xl text-sm sm:text-base"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
          ...
          >
            <div className="text-center mb-8">
              ...
              <p className="text-slate-600 dark:text-slate-400 text-lg sm:text-xl max-w-2xl mx-auto">
                Erkunden Sie die Mercedes-Benz C-Klasse in beeindruckenden 3D-Details
              </p>
            </div>
            <div className="max-w-6xl mx-auto">
              <div className="relative group">
                {/* Subtle glow effect on hover */}
                <div className="absolute -inset-1 bg-gradient-to-r from-blue-500/20 via-transparent to-blue-500/20 rounded-3xl blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                <MinimalCarViewer 
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 mb-12">
          ...
          >
            <div className="text-4xl sm:text-5xl font-black text-blue-600 dark:text-blue-400 mb-2">{generations.length.toLocaleString()}</div>
            <div className="text-sm font-bold text-blue-700 dark:text-blue-300 uppercase tracking-wide">Generationen</div>
          </motion.div>
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.15 }}
            className="bg-gradient-to-br from-slate-50 via-slate-100 to-slate-50 dark:from-slate-950/40 dark:via-slate-900/30 dark:to-slate-950/40 rounded-2xl p-5 sm:p-6 border-2 border-slate-200 dark:border-slate-900/30 shadow-lg hover:shadow-xl transition-shadow text-center"
          >
            <div className="text-4xl sm:text-5xl font-black text-slate-600 dark:text-slate-400 mb-2">{totalFaultsCount.toLocaleString()}</div>
            <div className="text-sm font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wide">Fehlerlösungen</div>
            <div className="text-xs text-slate-600/70 dark:text-slate-400/70 mt-1">Alle Generationen</div>
          </motion.div>
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            ...
            <div className="text-sm font-bold text-blue-700 dark:text-blue-300 uppercase tracking-wide">Gesamt produziert</div>
          </motion.div>
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.3 }}
            className="bg-gradient-to-br from-gray-50 via-gray-100 to-gray-50 dark:from-gray-950/40 dark:via-gray-900/30 dark:to-gray-950/40 rounded-2xl p-5 sm:p-6 border-2 border-gray-200 dark:border-gray-900/30 shadow-lg hover:shadow-xl transition-shadow text-center"
          >
            <div className="text-4xl sm:text-5xl font-black text-gray-600 dark:text-gray-400 mb-2">
              {generations.filter(g => g.year_start && g.year_start >= 2010).length}
            </div>
            <div className="text-sm font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wide">Modern (2010+)</div>
          </motion.div>
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            ...
            className="bg-gradient-to-br from-slate-50 via-slate-100 to-slate-50 dark:from-slate-950/40 dark:via-slate-900/30 dark:to-slate-950/40 rounded-2xl p-5 sm:p-6 border-2 border-slate-200 dark:border-slate-900/30 shadow-lg hover:shadow-xl transition-shadow text-center"
          >
            <div className="text-4xl sm:text-5xl font-black text-slate-600 dark:text-slate-400 mb-2">
              {generations.filter(g => !g.year_end).length}
            </div>
            <div className="text-sm font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wide">Aktuelle Produktion</div>
          </motion.div>
        </div>
        {/* Search Bar */}
        {generations.length > 3 && (
          <div className="mb-8 sm:mb-12">
            <div className="relative max-w-2xl mx-auto">
              <div className="absolute inset-0 bg-gradient-to-r from-blue-500/10 via-transparent to-blue-500/10 rounded-lg sm:rounded-xl blur-xl"></div>
              <input
                type="text"
                placeholder="Generationen suchen..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="relative w-full px-4 sm:px-6 py-3 sm:py-4 text-base sm:text-lg border-2 border-slate-300 dark:border-slate-700 rounded-lg sm:rounded-xl bg-white dark:bg-slate-900 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-600 focus:border-transparent transition-all shadow-lg min-h-[44px]"
              />
              <svg
                    ...
                    <div className="group relative bg-white dark:bg-slate-900 rounded-2xl overflow-hidden shadow-lg dark:shadow-2xl hover:shadow-2xl dark:hover:shadow-blue-900/20 transition-all duration-300 border border-slate-200 dark:border-slate-800 hover:border-blue-500 dark:hover:border-blue-500/50 h-full flex flex-col">
                      ...
                      <div className="p-6 sm:p-7 flex-grow flex flex-col relative z-10 bg-gradient-to-b from-transparent to-white/50 dark:to-slate-900/50">
                        <div className="flex items-start justify-between mb-3">
                          <h3 className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400/90 transition-colors leading-tight pr-2">
                            {generation.name}
                          </h3>
                          {generation.is_featured && (
                            <span className="ml-2 px-3 py-1 bg-blue-600 dark:bg-blue-500 text-white text-xs font-black rounded-full shadow-lg border-2 border-white/50 flex-shrink-0">
                              Empfohlen
                            </span>
                          )}
                        </div>
                          ...
                          <div className="flex items-center gap-2 mb-4">
                            ...
                            {generation.year_start && generation.year_end && (
                              <span className="ml-auto px-2.5 py-1 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 text-xs font-bold rounded-full">
                                {generation.year_end - generation.year_start + 1} Jahre
                              </span>
                            )}
                          </div>
                        )}
                        ...
                        <div className="mt-auto pt-4 border-t border-slate-200 dark:border-slate-700">
                          <div className="flex items-center justify-between">
                            ...
                            {generation.is_featured && (
                              <span className="px-2 py-0.5 bg-blue-100 dark:bg-blue-950/50 text-blue-700 dark:text-blue-400 text-xs font-bold rounded-full">
                                Empfohlen
                              </span>
                            )}
                          </div>
              ...
              {regularGenerations.map((generation, index) => (
                ...
                >
                  <Link
                    ...
                    className="group block h-full"
                  >
                    <div className="bg-white dark:bg-slate-900 rounded-2xl overflow-hidden shadow-md dark:shadow-xl hover:shadow-xl dark:hover:shadow-blue-900/10 transition-all duration-300 border border-slate-200 dark:border-slate-800 hover:border-blue-500 dark:hover:border-blue-500/50 h-full flex flex-col">
                      {generation.image_url ? (
                      ...
                      <div className="p-5 flex-grow flex flex-col bg-gradient-to-b from-transparent to-white/30 dark:to-slate-900/30">
                        <h3 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white mb-3 group-hover:text-blue-600 dark:group-hover:text-blue-400/90 transition-colors leading-tight">
                          {generation.name}
                        </h3>
